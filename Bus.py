class Bus:
    ...
    def get_specified_pq(self, base_mva: float = 100.0):
        """
        ...
        Q_spec = Q_gen - Q_load
        """
        p_gen = sum(g.calc_p(base_mva) for g in self.generators)
        # Generators usually control voltage (PV bus), thus Q_gen is unconstrained initially
        q_gen = 0.0 

        p_load = sum(l.calc_p(base_mva) for l in self.loads)
        q_load = sum(l.calc_q(base_mva) for l in self.loads)
